...
public class AccountClientTests {
	...
	public void getAccount() {
		...
		// template
		// - Run the test and ensure that it passes.
		Account account = restTemplate.getForObject(BASE_URL + "/accounts/{id}", Account.class, "0"); // Modify this
																										// line to use
																										// the
																										// restTemplate
	...
	public void addAndDeleteBeneficiary() {
		...
		URI returnedLocation = restTemplate.postForLocation(BASE_URL + "/accounts/1/beneficiaries", "David");
		// TODO-14: Retrieve the Beneficiary you just created from the location that was
		// returned
		Beneficiary newBeneficiary = restTemplate.getForObject(returnedLocation, Beneficiary.class); // Modify this line to use the restTemplate

		assertNotNull(newBeneficiary);
		assertEquals("David", newBeneficiary.getName());

		// TODO-15: Delete the newly created Beneficiary
		restTemplate.delete(returnedLocation);
		...
		HttpClientErrorException httpClientErrorException = assertThrows(HttpClientErrorException.class, () -> {
			...
			// was not successful.
			restTemplate.getForObject(returnedLocation, Beneficiary.class);
